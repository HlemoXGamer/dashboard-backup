<script setup>
import { assignDelivery, update } from "@/apis/logistics/orders";
...
const _updateOrder = async () => {
  ...
  try {
    ...
    status.value = currentStatus.value == "in_progress" || currentStatus.value == "paid" ? currentStatus.value : "";
    toast.success("Order updated successfully");
    actualStatus.value = currentStatus.value;
    emits('status', actualStatus.value);
    if (currentStatus.value === "in_progress") {
      ...
      //     order_id: orderId,
      //   });
      //   toast.su0ccess("SMS sent successfully");
      // }catch(err){
      //   toast.error(err);
      // }
    }
...
const statuses = computed(() => {
  let status = [
    ...
    { name: t("In Progress"), value: "in_progress", disabled: true },
  ...
  >
    ...
    <VCol>
      ...
      <VRow justify="space-between" align="center" class="mt-7">
        <VSelect
          :disabled="actualStatus !== 'ready_for_delivery'"
      ...
      <VRow justify="space-between" align="center" class="mt-7">
        ...
        <VBtn
          ...
          :disabled="status == currentStatus || currentStatus == 'paid'"
          >Save</VBtn
        >
      </VRow>
    </VCol>
