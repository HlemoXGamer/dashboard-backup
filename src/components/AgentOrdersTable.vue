<script setup>
import { get } from "@/apis/agent/deliveries";
...
<template>
  <VCard elevation="0" class="px-0 mb-0 pb-5 mt-6 pt-5" :loading="gettingOrders">
    <VDataTable
    ...
    <template #item.actions="{ item }"> 
      <IconBtn
        @click="
          $router.currentRoute.value.fullPath.includes('my-orders')
            ? $router.push(`/my-orders/${item.raw.id}`)
