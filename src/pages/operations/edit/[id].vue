<script setup>
import { show, update } from "@/apis/admin/operations";
import {
  emailValidator,
  integerValidator,
  kuwaitValidator,
...
const operationId = useRoute().params.id;
const form = ref({ name: "", phone: "", email: "" });
const refVForm = ref();

const _getOperation = async () => {
  await show(operationId).then((res) => {
...
const _updateOperation = async () => {
  refVForm.value?.validate().then(async ({ valid: isValid }) => {
    if (isValid) {
      loading.value = true;
      try {
        await update(operationId, form.value);
        loading.value = false;
...
<template>
  <VRow class="mt-4 px-4" justify="space-around">
    <VCol class="pt-0">
      ...
      >
        <p class="text-h4 pt-3 pb-2">{{ $t('Update Operation') }}</p>
        <VForm ref="refVForm" @submit.prevent="_updateOperation">
          <VCol>
