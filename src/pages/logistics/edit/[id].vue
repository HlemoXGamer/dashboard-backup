<script setup>
import { show, update } from "@/apis/admin/logistics";
import {
  emailValidator,
  integerValidator,
  kuwaitValidator,
...
const logisticId = useRoute().params.id;
const form = ref({ name: "", phone: "", email: "" });
const refVForm = ref();

const _getOperation = async () => {
  await show(logisticId).then((res) => {
...
const _updateLogistic = async () => {
  refVForm.value?.validate().then(async ({ valid: isValid }) => {
    if (isValid) {
      loading.value = true;
      try {
        await update(logisticId, form.value);
        loading.value = false;
...
<template>
  <VRow class="mt-4 px-4" justify="space-around">
    <VCol class="pt-0">
      ...
      >
        <p class="text-h4 pt-3 pb-2">{{ $t('Update Logistic') }}</p>
        <VForm ref="refVForm" @submit.prevent="_updateLogistic">
          <VCol>
