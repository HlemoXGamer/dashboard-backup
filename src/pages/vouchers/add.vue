...
import { create as adminCreate } from "@/apis/admin/vouchers";
import { get as getMarkterDiscounts } from "@/apis/markter/discounts";
import { create as markterCreate } from "@/apis/markter/vouchers";
...
const _getDiscounts = async () => {
  try {
    const { data }  = userRole == 'admin' ? await getAdminDiscounts() : await getMarkterDiscounts()
    discountTypes.value = data.data;
    discountTypesNames.value = data.data.map((discount) => discount.name);
  } catch (err) {
    toast.error(err);
  }
};

const _addVoucher = async () => {
    ...
    if (isValid) {
      ...
      try {
        ...
        form.value.v_type = discountTypes.value.filter(
          (type) => type.name == form.value.v_type_name,
        )[0].id;
        userRole == 'admin' ? await adminCreate(form.value) : await markterCreate(form.value)
        loading.value = false;
        toast.success("Voucher Added successfully");
      } catch (err) {
        Object.values(err.response.data.errors).forEach((error) => {
          toast.error(error[0].split(" v ").join(" "));
        });
        loading.value = false;
      }
    }
...
<template>
  <VRow class="mt-4 px-4" justify="space-around">
    <VCol class="pt-0">
      ...
      >
        <p class="text-h4 pt-3 pb-2">{{ $t('Add Voucher') }}</p>
        <VForm ref="refVForm" @submit.prevent="_addVoucher" class="w-100">
          <VCol>
