<script setup>
import { show, update } from "@/apis/admin/markters";
import {
  emailValidator,
  integerValidator,
  kuwaitValidator,
...
const marketerId = useRoute().params.id;
const form = ref({ name: "", phone: "", email: "" });
const refVForm = ref();

const _getMarketer = async () => {
  await show(marketerId).then((res) => {
...
const _updateMarketer = async () => {
  refVForm.value?.validate().then(async ({ valid: isValid }) => {
    if (isValid) {
      loading.value = true;
      try {
        await update(marketerId, form.value);
        loading.value = false;
...
<template>
  <VRow class="mt-4 px-4" justify="space-around">
    <VCol class="pt-0">
      ...
      >
        <p class="text-h4 pt-3 pb-2">{{ $t('Update Marketer') }}</p>
        <VForm ref="refVForm" @submit.prevent="_updateMarketer">
          <VCol>
