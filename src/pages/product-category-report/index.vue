...
const _getReports = (callFunction, query = "allProducts") => {
  gettingReports.value = true;
  callFunction(query)
    .then(({ data }) => {
...
const selectAllProducts = () => {
  if (form.value.product.length == products.value.length) {
    ...
    let allProducts = products.value
      .map((product) => product.id)
      .filter((id) => !form.value.product.includes(id));
...
const exportExcel = () => {
  ...
  if (
    Object.keys(excelFilters).includes("status") &&
    (Object.keys(excelFilters).includes("from") ||
      Object.keys(excelFilters).includes("to"))
  ) {
...
const hasProducts = computed(() => {
  return (
    (Object.keys(getFilters().filters).includes("product") ||
      Object.keys(getFilters().filters).includes("category")) &&
    isFiltered.value === true &&
    gettingReports.value === false
  );
});

const isOtherFieldsDisabled = computed(() => {
  timeKey.value += 1;
  return form.value.product.length === 0 && form.value.category.length === 0;
});
