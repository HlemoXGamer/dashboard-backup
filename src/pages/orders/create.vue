  ...
  <div>
    ...
    <VDialog v-model="noteDialog" persistent class="v-dialog-sm">
      ...
      <VCard :title="$t('Products Notes')">
        <VCardText v-for="(note, index) in form.products.find((product) => product.product_id == currentProduct.id).notes"
          class="pt-2">
          <AppTextField :label="`Note #${index + 1}`" class="flex-grow-1"
      ...
      <VCard title="Product Images">
        <VCardText class="d-flex flex-row align-center px-0 pb-5 pt-0 flex-wrap px-5">
          <div :style="form.products.find(
            ...
            <input @change="addFile" type="file" name="file" style="opacity: 0; height: 100%" class="h-100" multiple
              accept="image/png, image/gif, image/jpeg" :disabled="form.products.find(
                (product) => product.product_id == currentProduct,
              ).images.length ==
                form.products.find(
                  (product) => product.product_id == currentProduct,
      ...
      <VCard title="Additional Options">
        <VCardText>
          <VRow v-if="currentProduct">
            <VCol cols="6">
              <VSelect prepend-inner-icon="tabler-building-store" placeholder="Flavor" :label="$t('Flavor')"
                v-model="form.flavor" :items="products.find(product => product.id == currentProduct).flavors"
                item-value="id" item-title="name" variant="outlined" :return-object="false"
                class="flex-grow-1 my-1 mx-2" />
            </VCol>
            <VCol cols="6">
              <VSelect prepend-inner-icon="tabler-package" v-model="form.extra" item-title="name"
                :items="products.find(product => product.id == currentProduct).extras" item-value="id" variant="outlined"
                :label="$t('Extra')" :return-object="false" class="flex-grow-1 my-1 mx-2" multiple />
            </VCol>
          </VRow>
        </VCardText>
    ...
    <VRow class="mt-4 px-2" justify="space-around">
      <VForm ref="refVForm" @submit.prevent="_createOrder" class="w-100 d-flex">
        <VCol class="pt-0" :cols="$vuetify.display.smAndDown ? 12 : 8">
          ...
          <VForm ref="phoneForm" v-if="userRole == 'admin'" @submit.prevent="_getCustomer">
            <VCol class="px-5 rounded pb-7" style="background-color: rgb(var(--v-theme-surface))">
              ...
              <VCol>
                <VRow justify="space-between" align="start">
                  <AppTextField :rules="[
                    ...
                    integerValidator,
                    kuwaitValidator,
                  ]" :min="1" prepend-inner-icon="tabler-phone" style="width: 100%" class="pe-3 flex-grow-1"
                    v-model="customerPhone" :placeholder="t('Phone Number')">
                  </AppTextField>
                  <VBtn type="submit" :disabled="!customerPhone" :loading="searchLoading">{{ $t('Search') }}</VBtn>
                </VRow>
              </VCol>
            </VCol>
          </VForm>

          <VForm ref="phoneForm" v-if="userRole == 'restaurant'" @submit.prevent="_getRestaurantCustomer">
            <VCol class="px-5 rounded pb-7" style="background-color: rgb(var(--v-theme-surface))">
              <p class="text-h4 pt-3">{{ $t('Search by Phone Number') }}</p>
              <VCol>
                <VRow justify="space-between" align="start">
          ...
          <VCol v-if="userRole == 'admin' || userRole == 'restaurant'" class="mt-7 px-5 rounded pb-10"
            ...
            <VCol>
              <VRow justify="space-between" align="center" :class="!$vuetify.display.smAndDown ? 'flex-nowrap' : ''">
                <VCol cols="6" class="ma-0 py-0 pe-0 pe-2">
                  <VCombobox v-if="userRole == 'admin'" clearable prepend-inner-icon="tabler-building-community"
                    :loading="areasLoading" v-model="form.address_address_area" :return-object="false" :items="areas"
                    item-value="name" item-title="name" style="width: 100%" variant="outlined"
                    :rules="[requiredValidator]" :label="$t('Area')" class="flex-grow-1 my-1 w-100"
                    @update:model-value="updateStartEndTime()" />
                  <VCombobox v-if="userRole == 'restaurant'" clearable prepend-inner-icon="tabler-building-community"
                    :loading="areasLoading" v-model="form.address_address_area" :items="areas" item-value="name"
                    item-title="name" :return-object="false" style="width: 100%" variant="outlined"
                    :rules="[requiredValidator]" :label="($t('Area'))" class="flex-grow-1 my-1 w-100"
                    @update:model-value="updateStartEndTime()" />
                </VCol>
                <VCol cols="6" class="ma-0 py-0 pe-0 ps-2">
                  <VSelect clearable prepend-inner-icon="tabler-building-store" placeholder="Select a Branch"
                    :rules="[requiredValidator]" :label="$t('Branch')" :loading="branchesLoading" v-model="form.branch_id"
                    :items="availableBranches" item-value="id" :item-title="langIdentifier" style="width: 100%"
                    variant="outlined" :return-object="false"
                    :disabled="!form.address_address_area || form.address_address_area == ''"
                    @update:model-value="updateStartEndTime" class="w-100 flex-grow-1" />
                </VCol>
              </VRow>
            </VCol>
            <VRow class="mx-0 my-0 py-0 px-0" align="center" justify="space-between">
              ...
              <VRow justify="space-between" align="center" :class="!$vuetify.display.smAndDown ? 'flex-nowrap' : ''">
                <div v-if="form.is_pickup" class="w-100 flex-grow-1 d-flex">
                  <AppDateTimePicker :rules="[requiredValidator]" :disabled="!isAreaSelected"
                    ...
                    @update:model-value="updateStartEndTime" />
                  <AppDateTimePicker :rules="[requiredValidator]"
                    :disabled="!form.is_pickup || isClosed || !isBranchSelected" prepend-inner-icon="tabler-clock"
          ...
          <VCol class="mt-7 px-5 rounded pb-10" style="background-color: rgb(var(--v-theme-surface))">
            ...
            <VCol>
              <VRow justify="space-between" align="end">
                <VCombobox :disabled="!form.branch_id || (!form.delivery_date && form.is_pickup)"
                  prepend-inner-icon="tabler-package" :loading="productsLoading" v-model="selectedProduct"
                ...
                <AppTextField :disabled="!form.branch_id || (!String(form.delivery_date).length && form.is_pickup)"
                  :label="$t('Quantity')" type="number" class="flex-grow-1 mx-2 my-1 w-25" v-model="productCount">
                </AppTextField>
                <VBtn prepend-icon="tabler-plus" class="flex-grow-1 mx-2 my-1"
                  :disabled="productCount <= 0 || !selectedProduct" @click="addProduct">{{ $t('Add') }}</VBtn>
              </VRow>
              <VRow align="center">
          ...
          <VCol class="mt-7 px-5 rounded pb-10" style="background-color: rgb(var(--v-theme-surface))">
            ...
            <VCol>
              ...
              <VRow justify="space-between" align="end" class="mt-6">
                ...
                <AppTextField v-if="userRole == 'admin'" prepend-inner-icon="tabler-user-check" :label="$t('Agent')"
                  :placeholder="$t('Agent')" class="flex-grow-1 mx-2 my-1" v-model="form.agent"></AppTextField>
              </VRow>
            </VCol>
          </VCol>
            ...
            <VCol class="pt-0">
              ...
              <VRow class="mt-0">
                <VCol class="w-100 px-0 pt-0">
                  <AppTextField v-if="isAnEmployee" class="w-100 flex-grow-1" prepend-inner-icon="tabler-user-star"
                    :placeholder="$t('Enter Employee code')" v-model="employeeCode" style="width: 100%"
                    variant="outlined" />
                  <AppTextField v-if="!isAnEmployee" class="w-100 flex-grow-1" prepend-inner-icon="tabler-gift"
                    :placeholder="$t('Have a Voucher?')" v-model="vCode" style="width: 100%" variant="outlined" />
                  <VCheckbox @click="reset" :label="$t('Is An Employee')" v-model="isAnEmployee" class="mt-2" />
                  <VBtn v-if="!isAnEmployee && userRole == 'admin'" class="mt-3" :disabled="!vCode" block
                    :loading="voucherLoading" @click="_getVoucher">{{ $t('Apply Voucher') }}</VBtn>

                  <VBtn v-if="!isAnEmployee && userRole == 'restaurant'" class="mt-3" :disabled="!vCode" block
                    :loading="voucherLoading" @click="_getRestaurantVoucher">{{ $t('Apply Voucher') }}</VBtn>

                  <VBtn v-if="isAnEmployee && userRole == 'admin'" class="mt-3" :disabled="!employeeCode" block
                    :loading="employeeLoading" @click="_checkEmployee">{{ $t('Check Employee') }}</VBtn>

                  <VBtn v-if="isAnEmployee && userRole == 'restaurant'" class="mt-3" :disabled="!employeeCode" block
                    :loading="employeeLoading" @click="_checkRestaurantEmployee">{{ $t('Check Employee') }}</VBtn>
                </VCol>
              </VRow>
              <VRow class="mt-5">
                <VCard class="w-100 px-2 py-2" variant="tonal"
                  ...
                  <VCardText>
                    ...
                    <VRow>
                      <VDivider :color="$vuetify.theme.current.dark ? '#B3BFFFFF' : 'primary'
                        "></VDivider>
                    </VRow>
                    ...
                    <VRow v-if="form.v_code || form.employee_code" align="center" justify="space-between" class="mt-5">
