...
import { get as getBranches, getServiceCost } from "@/apis/agent/branches";
import { check as checkEmployee } from "@/apis/agent/employees";
import {
  create as createOrder,
  checkPhone as getCustomer,
  checkCode as getVoucher,
} from "@/apis/agent/orders";
import { getOrderProducts as getProducts } from "@/apis/agent/products";
import {
  emailValidator,
  integerValidator,
  kuwaitValidator,
...
const totalAfterDiscount = computed(() => {
  ...
  return newTotal.value + (form.value.delivery_cost ?? 0.5);
});
...
const _createOrder = async () => {
  ...
  formData.append("agent_note", form.value.agent);
      ...
      <VCardText>
        <VRadioGroup v-model="selectedAddress">
          <VRadio
            ...
            :key="address.id"
            ::label="$t('`${address.area_name} ~ ${address.street_name}`')"
            :value="address.id"
          />
        </VRadioGroup>
      </VCardText>
    ...
    <VRow class="mt-4 px-2" justify="space-around">
      <VForm ref="refVForm" @submit.prevent="_createOrder" class="w-100 d-flex">
        <VCol class="pt-0" :cols="$vuetify.display.smAndDown ? 12 : 8">
          ...
          <VForm ref="phoneForm" @submit.prevent="_getCustomer">
            ...
            <VCol>
              ...
              <VRow class="mt-4">
                <VCol class="w-100 px-0">
                  ...
                  <VBtn v-if="!isAnEmployee" class="mt-3" :disabled="!vCode" block :loading="voucherLoading"
                    @click="_getVoucher">{{ $t('Apply Voucher') }}</VBtn>
                  <VBtn v-if="isAnEmployee" class="mt-3" :disabled="!employeeCode" block :loading="employeeLoading"
                    @click="_checkEmployee">{{ $t('Check Employee') }}</VBtn>
                </VCol>
              </VRow>
              <VRow class="mt-5">
                <VCard class="w-100 px-2 py-2" variant="tonal"
                  ...
                  <VCardText>
                    <VRow align="center" justify="space-between">
                      ...
                      <p class="mb-2">{{ form.delivery_cost ?? 0.5 }} KWD</p>
                    </VRow>
                    <VRow>
                    ...
                    <VRow align="center" justify="space-between" class="mt-5">
                      ...
                      <p class="mb-2">{{ Number(total + (form.delivery_cost ?? 0.5)).toFixed(1) }} KWD</p>
                    </VRow>
                    ...
                    <VRow v-if="form.v_code || form.employee_code" align="center" justify="space-between" class="mt-5">
                      ...
                      <p class="mb-0">{{ Number(totalAfterDiscount).toFixed(2) }} KWD</p>
                    </VRow>
                  </VCardText>
                </VCard>
              </VRow>
